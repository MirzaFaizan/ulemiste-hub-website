...
if [ "${INSTALL_ZSH}" = "true" ]; then
    ...
    if [ ! -d "${OH_MY_INSTALL_DIR}" ] && [ "${INSTALL_OH_MYS}" = "true" ]; then
        ...
        PLUGINS_PATH="${OH_MY_INSTALL_DIR}/custom/plugins"
...
git clone "https://github.com/zsh-users/zsh-autosuggestions" "${USER_RC_PATH}/.oh-my-zsh/custom/plugins/zsh-autosuggestions" 2>&1
echo "plugins=(
git
zsh-autosuggestions
)" >> ${USER_RC_PATH}/.zshrc

echo "source \$ZSH/oh-my-zsh.sh" >> ${USER_RC_PATH}/.zshrc

echo "eval \$(thefuck --alias)" >> ${USER_RC_PATH}/.zshrc
